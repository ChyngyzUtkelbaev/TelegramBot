...
bot = telebot.TeleBot('1255563137:AAFakcDT9gfTkCZizYy61jTCL9vaZBm_Sc8')
#http://t.me/dsawa_bot Ссылка на бота
...
def start(message):
    ...
    bot.send_message(message.chat.id, '''Здравстуйте,меня зовут Азамат!Я помогу вам быстро найти место,куда сходить🌃
...
def options(message):
    if message.text == 'Еда':
        ...
        bot.send_message(message.chat.id,
                         ...
                         n🔺Средний чек-500 cом. Живая музыка.Бизнес-ланчи от'''
        ...
        bot.send_message(message.chat.id,
                         ...
                         n🔺Средний чек-400 cом. Паназиатская'''
        ...
        bot.send_message(message.chat.id,
                         ...
                         n⚪️ 🔥Профессиональные актеры 🔥3 уровня'''
        ...
        bot.send_message(message.chat.id,
                         "⚪️Scorpion\n⚪️Пейнтбольный клуб\n⚪️Персональный инструктор обучит всему "
        ...
        bot.send_message(message.chat.id,
                         "⚪️The story\n⚪️Антикинотеатр\n⚪️Кабинки для компаний от 2 до 20 человек. От 200 "
                         "сом/час.\n⚪️Адрес-https://go.2gis.com/d213w" )
        bot.send_photo ( message.chat.id , "http://biglife.kg/uploads/Развлечение/The story/slide1.jpg" )
        bot.send_message ( message.chat.id ,
                           "⚪️Zoobishkek\n⚪️Реабилитационный зоопарк\n⚪️Спасенные счастливые животные, большую часть из "
                           "которых можно погладить.Цены смешные, а впечатлений на "
                           "миллион\n⚪️Адрес-https://go.2gis.com/djlf1" )
        bot.send_photo ( message.chat.id , "http://biglife.kg/uploads/Развлечение/Зоосмешарики/1.jpg" )
    elif message.text == 'Развитие' :
        bot.send_message ( message.chat.id ,
                           "📚Coffee arte gallery\n📚Арт-центр кофейня\n📚Заведение для любителей кофе и "
                           "искусства.Имеются картины знаменитых художников,музыкальные инструменты и приятная "
        ...
        bot.send_message ( message.chat.id ,
                           ...
                           "Баялинова\n📚Библиотека\n📚Доступно для инвалидов.Имеется маркет-плэйс.Огромное количество "
                           "книг\n📍Адрес-https://go.2gis.com/k1tncn" )
        bot.send_photo ( message.chat.id , 'https://pbs.twimg.com/media/EJeC8ArXYAAZVIK.jpg' )
        bot.send_message ( message.chat.id ,
                           "📚Мемориальный дом-музей им. М.В. Фрунзе\n📚Музей\n📚В этом музее можно ознакомится с "
                           "историей образования города Фрунзе со всеми предметами быта того "
        ...
        bot.send_message ( message.chat.id ,
                           "📚Asanbay Center\n📚Культурный центр\n📚Необычное пространство для нашего города.Очень "
                           "интересно внутри. Проводят совершенно разные по направленности "
        ...
        bot.send_message ( message.from_user.id ,
                           'Этот бот был создан начинающим программистом для мидтерма в Международном '
                           'университете '
                           'Ала-Тоо🙊Я очень надеюсь,что мой бот был хоть чем-то полезен вам и вы нашли для себя '
        ...
        bot.send_message ( message.from_user.id , '''Тогда вот 7 идей,чем ты можешь заняться🏡
...
Любые изменения окружающего пространства идут на пользу клеткам головного мозга, улучшают память и настроение. 

7. Сделать планку✔''' )
    # else:
    # bot.send_message(message.chat.id, 'Извини,я тебя не понимаю😿Напиши мне что-то из меню ниже👇')
...
sezim_list = [ '🔺Chicken star\n🔺Арт-кафе\n🔺Средний чек-500 cом.Живая '
               ...
               "📚Мемориальный дом-музей им. М.В. Фрунзе\n📚Музей\n📚В этом музее можно ознакомится с "
               "историей образования города Фрунзе со всеми предметами быта того "
               "времени.\n📍Адрес-https://go.2gis.com/sc4h" , "⚪️Zoobishkek\n⚪️Реабилитационный зоопарк\n"
